...
Class Template {
        ...
        function __construct($registry) {
		  ...
		  $this->TF = ROOT_PATH.'public/tpl/face'.DIRSEP;
        }
		...
		function show() {
          ...
          if($this->registry['f_404']){
				...
				if(!$this->vars['no_tpl']):
					...
					$this->TF = ROOT_PATH.'public/tpl'.DIRSEP.$this->vars['tpl'].DIRSEP;
					$this->front_path = '/public/tpl/'.$this->vars['tpl'].'/';
		...
		function content(){
			if(isset($this->vars['c'])){
				require($this->TF.'blocks/content/'.$this->vars['c'].'.html');
			}
		}

		private function item_rq($name,$a = NULL){
			require($this->TF.'blocks/item/template/'.$name.'.html');
		}
