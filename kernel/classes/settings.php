<?
	Class Settings{
		...
		private function item_rq($name,$a = NULL){
			require($this->registry['template']->TF.'blocks/item/settings/'.$name.'.html');
		}
		...
		public function mk_sitemap_xml(){
			...
			$qLnk = mysql_query("
								...
								");
			while($g = mysql_fetch_assoc($qLnk)){
				$pages[$g['parent_level_alias']]['modified'] = $g['parent_level_modified'];
				$pages[$g['parent_level_alias']]['ch'][$g['level_alias']]['modified'] = $g['level_modified'];
				$pages[$g['parent_level_alias']]['ch'][$g['level_alias']]['ch'][$g['alias']]['modified'] = $g['modified'];
			}
			...
			$qLnk = mysql_query("
								...
								");
			while($g = mysql_fetch_assoc($qLnk)){
				$pages['growers']['modified'] = '';
				$pages['growers']['ch'][$g['alias']]['modified'] = $g['modified'];
			}
			$qLnk = mysql_query("SELECT MAX(modified) FROM growers;");
			$pages['growers']['modified'] = mysql_result($qLnk,0);
			...
			$qLnk = mysql_query("
								...
								");
			while($g = mysql_fetch_assoc($qLnk)){
				$pages['articles']['modified'] = '';
				$pages['articles']['ch'][$g['alias']]['modified'] = $g['modified'];
			}
			$qLnk = mysql_query("SELECT MAX(modified) FROM articles;");
			$pages['articles']['modified'] = mysql_result($qLnk,0);


			//новости
			$qLnk = mysql_query("
								SELECT
									news.date AS modified,
									news.alias
								FROM
									news
								WHERE
			...
			while($g = mysql_fetch_assoc($qLnk)){
				$pages['news']['modified'] = '';
				$pages['news']['ch'][$g['alias']]['modified'] = $g['modified'];
			}
			$qLnk = mysql_query("SELECT MAX(date) FROM news;");
			$pages['news']['modified'] = mysql_result($qLnk,0);
			...
			$qLnk = mysql_query("
								...
								");
			while($g = mysql_fetch_assoc($qLnk)){
				$pages[$g['alias']]['modified'] = $g['modified'];
			}
			...
			//главная страница
			$qLnk = mysql_query("SELECT MAX(news.date) FROM news WHERE news.published = 1 AND DATE(news.date) <= DATE(NOW());");
			$main_page_date = mysql_result($qLnk,0);

			$this->sitemap_node_recrute(THIS_URL,$main_page_date,'daily');

			foreach($pages as $first_level_alias => $arr){

				if($first_level_alias!=''){
					$alias_f = ($first_level_alias!='index') ? THIS_URL.$first_level_alias.'/' : THIS_URL;
					$this->sitemap_node_recrute($alias_f,$arr['modified']);

					if(isset($arr['ch'])){
						foreach($arr['ch'] as $second_level_alias => $ch_arr){

							$alias_s = $alias_f.$second_level_alias.'/';
							$this->sitemap_node_recrute($alias_s,$ch_arr['modified']);

							if(isset($ch_arr['ch'])){
								foreach($ch_arr['ch'] as $third_level_alias => $th_arr){

									$alias_t = $alias_s.$third_level_alias.'/';
									$this->sitemap_node_recrute($alias_t,$th_arr['modified']);
		...
		private function sitemap_node_recrute($alias,$lastmod,$changefreq = 'weekly'){
